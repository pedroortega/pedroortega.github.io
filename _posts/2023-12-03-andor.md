...
Let's consider two hypotheses $$h \in \mathcal{H}$$ with i.i.d. likelihoods $$q(x|h)$$
and priors $$q(h)$$.
...
The resulting estimator $$\sum_h q(h) q(x \mid h)$$ is indeed the Bayesian estimator.

The second step is to understand how to update our beliefs when making an observation.
